...
const App = () => {
  ...
  if (showSplash) {
      return <SplashPage onAccept={handleAcceptSplash} />;
  }

  // 2. Maintenance Mode Logic
  const isMaintenance = gameState.config.maintenanceMode;
  ...
  if (isLoggedIn && isMaintenance && !isAdmin) {
      return (
          <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
              <div className="text-center p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl max-w-lg animate-in fade-in zoom-in">
                  <div className="w-24 h-24 bg-yellow-500/10 rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse border-4 border-yellow-500/20">
                  ...
                  <p className="text-slate-400 mb-6">
                      The house is reshuffling the deck.<br/>
                      Server is currently locked for upgrades.
                  </p>
                  <button 
                    onClick={logout} 
                    className="px-6 py-2 bg-slate-800 hover:bg-red-900/50 text-red-400 hover:text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 mx-auto"
                  >
      ...
      if (isMaintenance) {
          ...
          return (
              <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
                  ...
                  <div className="text-center p-8 bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-3xl shadow-2xl max-w-lg animate-in fade-in zoom-in relative z-10">
                      <div className="w-24 h-24 bg-yellow-500/10 rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse border-4 border-yellow-500/20">
                          <Icons.Construction size={48} className="text-yellow-500" />
                      </div>
                      <h1 className="text-4xl font-black text-white mb-2">UNDER MAINTENANCE</h1>
                      ...
                      >
                          <Icons.Lock size={12}/> STAFF ACCESS
                      </button>
                  </div>
              </div>
          );
